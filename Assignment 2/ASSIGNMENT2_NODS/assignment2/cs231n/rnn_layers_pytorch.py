...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    #next_h = tanh(x*Wx +prev_h*Wh +b) #rnn formula (current + previous+ bias)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    N,T,D = x.shape
    #N: batch size, T:timesteps. D:dimenions or
    #N setences, T word/sentence, D features/word
    _, H = h0.shape #already got N
    h = torch.zeros((N, T, H), dtype=x.dtype, device=x.device)
    curr_mem = h0
    for t in range(T):
      x_t = x[:, t, :] #current word for all sentences
      curr_mem = rnn_step_forward(x_t, curr_mem, Wx, Wh, b)
      h[:,t,:] = curr_mem
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    out = W[x]# Advanced indexing, Replaces each integer in x with its matching row vector from W. 
    #Shape expands to (N, T, D). 
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return out
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    Note that a sigmoid() function has already been provided for you in this file. <-LIE
    ...
    #############################################################################
    gate_outputs = x @ Wx + prev_h @ Wh + b #shape is (N,4H)
    a_i, a_f, a_o, a_g = torch.chunk(gate_outputs, 4, dim=1)#dim=1 as each col corresp to diff gate
    i = torch.sigmoid(a_i) #input gate (N,H)
    f = torch.sigmoid(a_f) #forget gate
    o = torch.sigmoid(a_o) #output gate
    g = torch.tanh(a_g) #cadidate memory
    next_c = (f*prev_c)+(i*g)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    _, H = h0.shape
    h = torch.zeros((N, T, H), dtype=x.dtype, device=x.device)
    curr_h = h0 #current hidden state
    curr_c = torch.zeros_like(h0) #current cell state initialized to 0
    for t in range(T):
      x_t = x[:, t, :] #current word in the timestamp
      curr_h, curr_c = lstm_step_forward(x_t, curr_h, curr_c, Wx, Wh, b)
      h[:,t,:] = curr_h
      #cell state isnt returned
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
