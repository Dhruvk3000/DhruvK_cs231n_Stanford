...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            ##################################################################
            down_block = nn.ModuleList([ #ModuleList of 3 blocks [ResnetBlock, ResnetBlock, Downsample].
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        # gives pure essence of the prompt
        # Get the conditional prediction (WITH the text prompt)
        out_cond = self.forward(x, time, model_kwargs)

        #make a copy of the arguments dictionary and explicitly set the text embedding to None
        kwargs_uncond = copy.deepcopy(model_kwargs)
        kwargs_uncond["text_emb"] = None 

        #get the unconditional prediction (WITHOUT the text prompt)
        out_uncond = self.forward(x, time, kwargs_uncond)

        # Extrapolate to amplify the condition!
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for upsample, block1, block2 in self.ups:
            x = upsample(x)
            # pop saved skip connection and concatenate along channels (dim=1)
            x = torch.cat([x, skips.pop()], dim=1)
            x = block1(x, context)
            # pop saved next skip connection and concatenate
            x = torch.cat([x, skips.pop()], dim=1)
            x = block2(x, context)
        ##################################################################
