...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        #need position column vector i from 0 to max_length-1
        #calculated in init to reduce computational complexity during forward pass
        position = torch.arange(max_len).unsqueeze(1) #unsqueeze to add columm. Shape is (max_len,1)
        #need row vector j of even indices, step size would be 2
        row = torch.arange(0, embed_dim, 2) #shape (embed_dim//2)
        scaling_factor = 1/(10000**(row/embed_dim))
        #Because of automatic prepending, PyTorch treats the 1D row array exactly as if it were a 2D row vector of shape
        grid = position * scaling_factor #shape: (max_len,embed_dim//2)
        pe[0,:,0::2]= torch.sin(grid) #for slice 0 in batch, for all rows (words), starting from 0 to end with step size 2 (even cols)
        pe[0,:,1::2]= torch.cos(grid)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def forward(self, x):
        ...
        ############################################################################
        #need to slice self.pe to get the first S elements in the sequence dimension.
        #self.pe has dims (1, max_len, embed_dim)
        pe_slice = self.pe[:,0:S,:] #(1, S, D)
        x += pe_slice
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        H = self.n_head
        D_h = self.head_dim #E= H * D_h
        #Linear projs
        Q = self.query(query)
        K = self.key(key)
        V = self.value(value)
        #Multi head split
        Q = Q.view(N,S,H,D_h).permute(0,2,1,3) #Q (N,S,E) -> (N,S,H,D_h) -> (N,H,S,D_h) per word view to per head view
        K = K.view(N,T,H,D_h).permute(0,2,1,3) #K (N,T,E) -> (N,T,H,D_h) -> (N,H,D_h,T)
        V = V.view(N,T,H,D_h).permute(0,2,1,3) #V (N,T,E) -> (N,T,H,D_h) -> (N,H,D_h,T)
        K_transp = K.transpose(-2,-1) #K (N,H,D_h,T) -> (N,H,T,D_h)
        #Attention scores
        Scores = (torch.matmul(Q,K_transp))/(D_h**0.5) #dot_p/scaling factor
        #multiply by attention masks
        if attn_mask is not None:
          Scores = Scores.masked_fill(attn_mask==0,-1e9)
        #softmax + dropout
        attn_weights = torch.softmax(Scores, dim=-1) #softmax across dim T shpae is (N,H,S,T)
        attn_weights = self.attn_drop(attn_weights)
        #Multiply by V, get context vectors
        context = attn_weights @ V #(N,H,S,T * N,H,T,D_h) -> (N,H,S,D_h)
        context = context.permute(0,2,1,3) #(N,H,S,D_h)-> (N,S,H,D_h)
        context = context.reshape(N,S,E)
        #final projection
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1):
        ...
        self.cross_attn = MultiHeadAttention(input_dim, num_heads, dropout) #internally the same, depends on the data we pass to them
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        Hp = H//self.patch_size #int division
        Wp = W//self.patch_size
        P = self.patch_size
        #step 1 split
        img = x.reshape(N,C,Hp,P,Wp,P)
        #step 2 rearrange
        img = img.permute(0,2,4,1,3,5)
        #step 3 flatten into seq
        num_patches = Hp*Wp
        patch_dim = C*P*P
        img = img.reshape(N, num_patches, patch_dim)
        #step 4 Linear proj
        out = self.proj(img)
        ############################################################################
        #                             END OF YOUR CODE                             #
