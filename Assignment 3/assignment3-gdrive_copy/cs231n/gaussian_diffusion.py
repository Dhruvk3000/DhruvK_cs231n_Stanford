...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        a = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        b = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        # Rearranged formula solving for x_start
        x_start = (x_t - b * noise) / a
        ####################################################################
        return x_start
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # Model predicts the noise (or x_start)
        pred = self.model(x=x_t, time=t, model_kwargs=model_kwargs)
        
        # Recover x_start
        if self.objective == "pred_noise": #model predicts noise and subtracts from noisy img
            x_start = self.predict_start_from_noise(x_t, t, noise=pred)
        else:
            x_start = pred
            
        # clamp x_start to [-1, 1] to prevent pixel values from exploding
        x_start = x_start.clamp(-1.0, 1.0)
        
        # Get the mean and standard deviation to take 1 step backwards
        mean, std = self.q_posterior(x_start=x_start, x_t=x_t, t=t)
        
        # 5. Sample from the distribution: mean + std * random_noise
        # (Exception: if t==0, final image so no noise)
        if t_int > 0:
            random_noise = torch.randn_like(x_t)
        else:
            random_noise = torch.zeros_like(x_t)
            
        x_tm1 = mean + std * random_noise
        ##################################################################
    ...
    def q_sample(self, x_start, t, noise):
        ...
        ####################################################################
        #reparam trick: # x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * epsilon
        a = extract(self.sqrt_alphas_cumprod, t, x_start.shape) #alpha_bar for time t
        b = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) #(1-alpha_bar) fpr time t
        # apply formula
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        ####################################################################
        x_t = self.q_sample(x_start=x_start, t=t, noise=noise) #gen noisy img
        pred = self.model(x=x_t, time=t, model_kwargs=model_kwargs) # img+timestamp to unet model
        #WEighted MSE Formula: mean( weight * (prediction - target)^2 )
        loss = torch.mean(loss_weight * (pred - target) ** 2)
