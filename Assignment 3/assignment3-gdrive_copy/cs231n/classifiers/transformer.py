...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # step 1: embeding caption and add positional encoding
        #raw input -> embeded text with PE
        tgt = self.embedding(captions)
        tgt = self.positional_encoding(tgt)

        # step 2: process img features: Visual Projection (features -> memory)
        #converting img features to word dimentions (N, img_d)->(N, W) W is model's internal vocab dim
        #unsqueezed (N,W)->(N,1,W) as attn expects (N,S,W)
        memory = self.visual_projection(features).unsqueeze(1)

        #step 3: as we pass entire seq at once to gpu, create mask to avoid model to look at future tokens
        #it would like a lower triangular matrix
        #[1,0,0] Word1 sees only Word1
        #[1,1,0] Word2 sees Word1 and 2
        #[1,1,1] Word3 can see all prev words
        ...
        #step 5: project to english vocab V  (N, T, W)->(N, T, V)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #step 2 pe on patches
        x = self.positional_encoding(x)
        #step 3 encoder forward
        x = self.transformer(x) #Ouput (N,S,D)
        #step 4 avg pooling across S (to get (N,D for linear classifier))
        x = torch.mean(x,axis=1)
        #step 5 final linear classifier
        logits = self.head(x)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
