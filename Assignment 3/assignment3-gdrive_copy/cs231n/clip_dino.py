...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    norm_text = text_features/torch.linalg.norm(text_features, dim=1, keepdims=True) #normalize and get (N,D)
    norm_img = image_features/torch.linalg.norm(image_features, dim=1, keepdims=True) #normalize and get (M,D)
    similarity = norm_text @ norm_img.t() #(N,M)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    #get similarities
    sim = get_similarity_no_loop(text_features, image_features) #(N classes, M images)
    best_indices = torch.argmax(sim, dim=0) #(M,) indices
    pred_classes = [class_texts[i] for i in best_indices] #best class label per image
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # normalize them
        self.image_features /= torch.linalg.norm(self.image_features, dim=1, keepdim=True)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        # opp of zero shot classifier
        # query text and return the best images
        # step 1 Tokenize the text query
        text_token = clip.tokenize([query]).to(self.device)
        # step 2 Extract the text feature using the model from the backpack
        text_feature = self.clip_model.encode_text(text_token)
        # step 3 normalize text features
        text_feature /= torch.linalg.norm(text_feature, dim=1, keepdim=True)
        # step 4 calc sim
        sim = text_feature @ self.image_features.t() #(1, D) @ (D, M)
        # step 5 get top 'k' indices
        _, indices = sim.topk(k, dim=1) #returns a tuple: (values, indices)
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        ############################################################################
        # pytorch model: inp_dim (384) to num_classes
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for _ in range(num_iters):
          ...
          out = self.model(X_train) #forward pass
          loss = self.criterion(out, Y_train) #compute loss
          loss.backward() #backward pass
          self.optimizer.step() #optim step
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def inference(self, X_test):
        ...
        out = self.model(X_test) #forward pass (N, num_classes)
        pred_classes = torch.argmax(out, dim=1) #label with highest prob
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes